...
class Cassette(
    ...
    def __init__(
        ...
        Args:
            ...
            end_blocks_distance (list, L optional): List of L distances,
                ...
                end_blocks_length and end_blocks_distance are not None.
                Defaults to None.
            start_blocks_magnetization (list, Lx3, optional): Nested list of
                L 3-vectors determining the magnetizations of start blocks.
                If given, this list will override any magnetization given by
                a magnetization_list read from .json file or generated by the
                get_ideal_magnetization_list call in init_radia_object.
                In Tesla, defaults to None.
        ...
        Raises:
            ...
            ValueError: If start_blocks_length, start_blocks_distance and
                start_blocks_magnetization (if not None) do not have the same
                number of elements.
            ValueError: If end_blocks_length, end_blocks_distance and
                end_blocks_magnetization (if not None) do not have the same
                number of elements.
    ...
    @property
    def start_blocks_magnetization(self):
        """List of magnetizations of blocks in the start of the cassette."""
        return _deepcopy(self._start_blocks_magnetization)

    @property
    def end_blocks_magnetization(self):
        """List of magnetizations of blocks in the end of the cassette."""
    ...
    def create_radia_object(
        ...
        if self.hybrid:
            if is_pole_list is None:
                mag0 = magnetization_list[self.nr_start_blocks]
                # Check if first core block has (mx,my,mz) with |my|>|mz|
                ...
                self._is_pole_list = is_pole_list
                if is_pole_list[self.nr_start_blocks]:
                    core_lengths = [self._pole_length, block_length]*int(
                            self.nr_core_blocks/2)
                else:
                    core_lengths = [block_length, self._pole_length]*int(
                            self.nr_core_blocks/2)
                length_list = _utils.flatten([
                    self._start_blocks_length, core_lengths, self._end_blocks_length
                ])
        else:
